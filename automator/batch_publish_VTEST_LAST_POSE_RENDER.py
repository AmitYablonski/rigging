...
class Publish_QA(object):
    ...
    def run(self):
        ...
        print(" PRE IMPORT DEADLINE")
        try:
            self.import_data_deadline()
            print("IMPORT DEADLINE")
            self.execute()
    ...
    def execute(self):
        ...
        asset_path = os.path.normpath(self.job_data['jobs'][job_key][asset_key][u"path"]).split(os.sep)
        asset_name = asset_path[-1]
        asset_type = asset_path[-2]

        from MBA_SE02.internal.publish.apps import publisher_core
        print("IMPORT publisher_core")
        data = {
            "root": root,
            "project": "MBA_SE02",
